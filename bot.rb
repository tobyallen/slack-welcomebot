...
class Events
  ...
  def self.message(team_id, event_data)
    ...
    unless user_id == $teams[team_id][:bot_user_id]
      ...
      #Check if sent in DM or Open Channel
      channel_id = event_data['channel']
      unless channel_id.starts_with?("C")
        channel_id = user_id
      end
      ...
      # Show Welcome Text
      if event_data['text'] && event_data['text'].scan(/help|tutorial/i).any?
        $teams[team_id]['client'].chat_postMessage(
          as_user: 'true',
          channel: channel_id,
          text: "This is a simple welcome bot. Not much fancy stuff included.\n"\
            "Help will display this message.\n"\
            "Joke will tell you a joke.\n"
        )
      end

      # Respond with a random dad joke.
      if event_data['text'] && event_data['text'].scan(/joke|funny|dad/i).any?
